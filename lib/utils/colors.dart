import 'package:flutter/material.dart';

class AppColor {
  static const Color lightScafoldColor = Colors.white;
  static const Color lightPrimary = Color(0xff181059);
  static const Color lightCardColor = Color.fromARGB(106, 250, 250, 250);
  static const Color darkScafoldColor = Color.fromARGB(255, 9, 3, 27);
  static const Color darkPrimary = Color.fromARGB(255, 94, 75, 236);
  static const Color darkCardColor = Color.fromARGB(255, 13, 6, 37);
  static const Color deepredColor = Color.fromARGB(255, 136, 14, 5);
  static const Color greyColor = Colors.grey;
  static const Color redColor = Colors.red;
  static const Color blueColor = Colors.blue;
  static const Color blackColor = Colors.black;
  static const Color lightBluekColor = Colors.lightBlue;
  static const Color kPrimaryColor = Color(0xff0DF5E3);
  static const Color kGreyColor = Color(0xff8D8D8D);
}
